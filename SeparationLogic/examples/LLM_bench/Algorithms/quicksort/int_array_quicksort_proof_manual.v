...
Proof.
  ...
  - rewrite <- Zlength_correct in H9.
    rewrite !Zlength_replace_Znth in H9.
    lia.
Qed.
...
Proof.
  ...
  destruct H8 as [Hperm [Hsame [Hpivot [Hle Hgt]]]].
  split.
  - exact Hperm.
  - split.
    + exact Hsame.
...
Proof.
  ...
  - assert (Hj : j = high_pre) by lia.
    ...
    {
      rewrite <- Zlength_correct in H8.
      rewrite !Zlength_replace_Znth in H8.
      lia.
    }
    eapply partitioned_at_after_final_swap; eauto; lia.
  - destruct H7 as [_ [Hsame _]].
    assert (Hlenarr : high_pre < Zlength l1_2).
    {
      rewrite <- Zlength_correct in H8.
      rewrite !Zlength_replace_Znth in H8.
...
Proof.
  ...
  Exists l1_4.
  entailer!.
  - pose proof H4 as Hsame23_len.
    pose proof H0 as Hsame34_len.
    destruct Hsame23_len as [Hlen23 _].
    destruct Hsame34_len as [Hlen34 Heq34].
    assert (Hlen4 : Zlength l1_4 = n_pre).
    { rewrite Zlength_correct. exact H17. }
