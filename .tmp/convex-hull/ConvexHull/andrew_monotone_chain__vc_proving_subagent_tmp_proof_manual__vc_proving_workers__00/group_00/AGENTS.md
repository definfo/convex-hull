...
- Do NOT inspect compile-only dependency paths with shell tools or editor reads. In particular, do not run `cat`, `sed`, `rg`, `grep`, `find`, `ls`, `head`, `tail`, or similar commands on:
  ...
  - `/home/definfo/Develop/qcp/convex-hull/.tmp/convex-hull/ConvexHull/andrew_monotone_chain__vc_proving_subagent_tmp_proof_manual__vc_proving_workers__00/group_00/case_deps/convex_hull`
...
## Assigned Goals
- `proof_of_cmp_xy_return_wit_1` (`goal_01__proof_of_cmp_xy_return_wit_1.v`)
- `proof_of_cmp_xy_return_wit_2` (`goal_02__proof_of_cmp_xy_return_wit_2.v`)
- `proof_of_cmp_xy_return_wit_3` (`goal_03__proof_of_cmp_xy_return_wit_3.v`)
- `proof_of_cmp_xy_return_wit_4` (`goal_04__proof_of_cmp_xy_return_wit_4.v`)
- `proof_of_cmp_xy_return_wit_5` (`goal_05__proof_of_cmp_xy_return_wit_5.v`)

## Proof Group Notes
- proof_group_id: cmp_xy_returns
- grouping_source: vc-checking-group-plan
- representative_witness: proof_of_cmp_xy_return_wit_1
- natural_language_proof_pattern: Unfold the comparator postcondition and use branch inequalities to prove the exact return value.
- shared_helper_candidates: empty
- proving_hints: Use existing comparator lemmas where convenient; otherwise split pures and discharge by integer order reasoning.
