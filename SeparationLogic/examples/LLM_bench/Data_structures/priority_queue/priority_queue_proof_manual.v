...
Proof.
  pre_process.
  replace (parent - 0) with parent in H by lia.
  replace (child - 0) with child in H by lia.
  pose proof
    (push_loop_stop_to_result l l_cur_2 n_pre child parent x_pre H10 H2 H7 H)
    as Hpush.
  Exists l_cur_2.
  split_pure_spatial.
  - cancel (IntArray.seg heap_pre 0 (n_pre + 1) l_cur_2).
...
Proof.
  ...
  subst child.
  pose proof (push_loop_root_to_result l l_cur n_pre x_pre H6) as Hpush.
  Exists l_cur.
  split_pure_spatial.
...
Proof.
  pre_process.
  pose proof H as Hpush_old.
  pose proof H8 as Hbuild_old.
  destruct H as
  ...
  - split_pures.
    ...
    + rewrite H7.
      pose proof (H11 i ltac:(lia)) as Hrange_i.
...
Proof.
  pre_process.
  destruct H5 as [_ [_ [Hheap_len [_ [_ _]]]]].
  entailer!.
  - rewrite Zlength_sublist0 by (rewrite Hheap_len, H1; lia).
    lia.
  - rewrite Znth_sublist0 by lia.
    symmetry.
    exact H4.
  - rewrite H4.
    pose proof (H8 i ltac:(lia)) as Hrange_i.
    lia.
  - rewrite H4.
    pose proof (H8 i ltac:(lia)) as Hrange_i.
    lia.
  - apply sublist_prefix_preserves_max_heap.
    + lia.
    + rewrite Hheap_len, H1.
      lia.
    + exact H7.
  - rewrite Znth_sublist0 by lia.
    pose proof (H8 0 ltac:(lia)) as Hrange_0.
    lia.
  - rewrite Znth_sublist0 by lia.
    pose proof (H8 0 ltac:(lia)) as Hrange_0.
    lia.
  ...
  - rewrite Znth_sublist0 by lia.
    ...
    exact Hrange_i.
  - intros idx Hidx.
    eapply sublist_prefix_range; try exact H8; lia.
Qed. 
...
Proof.
  ...
  - split_pures.
    all: dump_pre_spatial; unfold PriorityQueuePrefix in *; intuition auto; try lia.
    + pose proof (H3 0 ltac:(lia)) as [? ?]; lia.
    + pose proof (H3 0 ltac:(lia)) as [? ?]; lia.
    + apply H5; lia.
Qed. 
...
Proof.
  ...
  pose proof
    (pop_loop_swap_down_preserve l heap_l n_pre idx largest H13 H12 H)
    as [Hidx_largest Hloop].
  assert (Hidx_range : 0 <= idx < Zlength heap_l).
  {
    pose proof H13 as Hstate.
...
Proof.
  pre_process.
  pose proof
    (pop_ready_final_write_to_result_with_range l heap_l n_pre ret H7 H2 H3 H8)
