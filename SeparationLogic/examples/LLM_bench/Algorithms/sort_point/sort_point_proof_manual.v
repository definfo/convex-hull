...
Proof.
  pre_process.
  pose proof H6 as Hflat.
  destruct H6 as [Hflat_len _].
  replace
...
Proof.
  ...
  { unfold PointCoordsBound in H6 |- *.
    inversion H6; subst.
    assumption.
  }
  pose proof (flat_points_lookup_point flat pts_l high_pre Hhigh H5) as Hpivot.
...
Proof.
  pre_process.
  assert (Hjrange : 0 <= j < Zlength pts_cur_2) by (rewrite H16; lia).
  assert (Hi1 : 0 <= i + 1 < Zlength pts_cur_2) by (rewrite H16; lia).
  assert (Hhighrange : 0 <= high_pre < Zlength pts_cur_2) by (rewrite H16; lia).
  assert (
    Hjpt :
  ...
  {
    pose proof (PolarCmpResult_nonpos_le _ _ _ _ H3 H2) as Hle.
    rewrite <- Hjpt in Hle.
    exact Hle.
  }
  ...
  destruct H21 as [Hperm [Hsame [Hpivot [Hleft Hmid]]]].
    ...
    + repeat split.
      ...
      * replace (Znth high_pre (point_swap_points pts_cur_2 (i + 1) j) default_point)
        ...
        -- symmetry.
           ...
           ++ lia.
           ++ lia.
      * destruct H as [Hflat_len _].
        exact Hflat_len.
      * destruct H as [_ Hflat_rec].
        exact Hflat_rec.
      * exact H1.
      * constructor.
        -- exact Hgxy.
...
Proof.
  ...
  assert (Hjrange : 0 <= j < Zlength pts_cur_2) by (rewrite H13; lia).
  assert (
    Hjpt :
      Znth j pts_cur_2 default_point =
      mk_point (Znth (2 * j) flat_cur_2 0) (Znth (2 * j + 1) flat_cur_2 0)
  ...
  {
    rewrite Hret1 in H0.
    pose proof (PolarCmpResult_one_implies_PolarLt_flip _ _ _ H0) as Hlt.
    rewrite <- Hjpt in Hlt.
    exact Hlt.
  }
  destruct H18 as [Hperm [Hsame [Hpivot [Hleft Hmid]]]].
  ...
  - split.
    + repeat split.
      ...
      * exact H13.
      * exact H14.
      * destruct H15 as [Hflat_len _].
        exact Hflat_len.
      * destruct H15 as [_ Hflat_rec].
        exact Hflat_rec.
      * exact H16.
      * exact H17.
    + unfold PointPartitionScanInv.
...
Proof.
  ...
  - entailer!.
    ...
    + destruct H17 as [_ [Hsame _]].
      eapply PointSameOutsideRange_trans.
      * exact Hsame.
      * apply PointSameOutsideRange_swap_inside; lia.
    + destruct H17 as [Hperm_scan _].
      eapply Permutation_trans.
      * exact Hperm_scan.
      * exact H0.
    + constructor; [exact Hgpbound | exact H1].
Qed. 
...
Proof.
  ...
  { rewrite H9. lia. }
  assert (Hgxy : CoordInBounds gx_pre /\ CoordInBounds gy_pre).
  { unfold PointCoordsBound in H13.
    inversion H13; subst.
    tauto.
  }
  pose proof (point_coords_bound_lookup pts_cur j Hjrange H12) as Hjcoords.
  pose proof (point_coords_bound_lookup pts_cur high_pre Hhigh H12) as Hpivotcoords.
  pose proof (flat_points_lookup_point flat_cur pts_cur j Hjrange H11) as Hlookupj.
  entailer!.
...
Proof.
  intros.
  pre_process.
  unfold PointMemoryModel in H5.
  destruct H5 as [pts_l [? [? ?]]].
  Exists pts_l.
  entailer!.
...
Proof.
  ...
  - entailer!.
    ...
    assert (Hlen2 : Zlength pts_out_2 = n_pre).
    {
      ...
      rewrite <- (Permutation_length H11).
      reflexivity.
    }
    assert (Hlen4 : Zlength pts_out_4 = n_pre).
    {
      rewrite <- Zlength_correct in H23.
      rewrite Hlenflat4 in H23.
      lia.
    }
    ...
    subst pts_in.
    specialize (H5 pts_out_2 H9 H10).
    destruct H5 as [Hperm23 [Hsame23 Hsorted23]].
    specialize (H1 pts_out_3 H3 H4).
    destruct H1 as [Hperm34 [Hsame34 Hsorted34]].
    ...
    + split.
      * assert (Hsame23_full : PointSameOutsideRange pts_out_2 pts_out_3 left_pre right_pre).
        ...
        eapply PointSameOutsideRange_trans.
        -- eapply PointSameOutsideRange_trans; [exact H12 | exact Hsame23_full].
        -- exact Hsame34_full.
...
Proof.
  ...
  - entailer!.
    ...
    assert (pts_in = pts_l) by (eapply FlatPoints_functional; eauto).
    subst pts_in.
    specialize (H1 pts_out_2 H6 H7).
    destruct H1 as [Hperm23 [Hsame23 Hsorted23]].
    split.
    + eapply Permutation_trans; [exact H8 | exact Hperm23].
    + split.
      * assert (Hsame23_full : PointSameOutsideRange pts_out_2 pts_out_3 left_pre right_pre).
        {
          eapply (PointSameOutsideRange_weaken
        ...
        eapply PointSameOutsideRange_trans; [exact H9 | exact Hsame23_full].
...
Proof.
  ...
  - entailer!.
    ...
    assert (pts_in = pts_l) by (eapply FlatPoints_functional; eauto).
    subst pts_in.
    specialize (H2 pts_out_2 H6 H7).
    destruct H2 as [Hperm23 [Hsame23 Hsorted23]].
    split.
    + eapply Permutation_trans; [exact H8 | exact Hperm23].
    + split.
      * assert (Hsame23_full : PointSameOutsideRange pts_out_2 pts_out_3 left_pre right_pre).
...
Proof.
  pre_process.
  assert (Hge : left_pre >= right_pre) by auto.
  match goal with
  | Hmem : PointMemoryModel _ _ _ |- _ =>
      destruct Hmem as [pts_l [Hlen [Hflat_mem Hbound_mem]]]
  end.
  Exists flat. Exists pts_l.
  split_pure_spatial.
  - cancel.
...
Proof.
  ...
  - rewrite <- H13.
    repeat rewrite Zlength_correct.
    rewrite (Permutation_length H4).
    reflexivity.
  - unfold FlatPoints in H2.
    tauto.
  - unfold FlatPoints in H2.
    tauto.
Qed. 
...
Proof.
  ...
  exists pts_out_2.
  repeat split; auto.
  - destruct (H2 _ H6 H7) as [Hperm _].
    rewrite <- H17.
    repeat rewrite Zlength_correct.
    rewrite (Permutation_length H8).
    rewrite (Permutation_length Hperm).
    reflexivity.
  - unfold FlatPoints in H0.
    tauto.
  - unfold FlatPoints in H0.
    tauto.
Qed. 
...
Proof.
  ...
  - rewrite <- H14.
    repeat rewrite Zlength_correct.
    rewrite (Permutation_length H5).
    reflexivity.
  - unfold FlatPoints in H3.
    tauto.
  - unfold FlatPoints in H3.
    tauto.
Qed. 
...
Proof.
  ...
  - cancel.
  - entailer!.
    + destruct (H1 pts_l H5 H6) as [Hperm [_ Hsorted]].
      unfold PolarSorted.
      ...
      eapply Hsorted; lia.
    + destruct (H1 pts_l H5 H6) as [Hperm _].
      exact Hperm.
Qed. 
