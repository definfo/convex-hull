...
Proof.
  ...
  {
    eapply PrefixArrayPrefix_entry_abs_bound; [exact H5 | | lia].
    intros idx Hidx.
    apply H6.
    lia.
  }
  assert (Hl_bound : -1000 <= Znth i l 0 <= 1000) by (apply H6; lia).
...
Proof.
  ...
  - split_pures.
    ...
    + dump_pre_spatial.
      ...
      {
        eapply PrefixArrayPrefix_entry_abs_bound; [exact H5 | exact H6 | lia].
      }
      lia.
Qed. 
...
Proof.
  ...
  split_pure_spatial.
  - repeat cancel.
  - unfold InitialFrontierState in H17.
    split_pures.
...
Proof.
  ...
  {
    unfold ValidNodeFields, ValidNode in H40; cbn in H40.
    destruct H40 as [_ [_ [_ [_ [_ [_ [_ [_ [Hvalue _]]]]]]]]].
    rewrite Hvalue.
    eapply PrefixSums_diff_int_bounds; eauto; lia.
  }
  assert (Hleft_int : -2147483648 <= left_value <= 2147483647).
  {
    unfold ValidNodeFields, ValidNode in H49; cbn in H49.
    destruct H49 as [_ [_ [_ [_ [_ [_ [_ [_ [Hvalue _]]]]]]]]].
    rewrite Hvalue.
    eapply PrefixSums_diff_int_bounds; eauto; lia.
  }
  assert (Hstart_lo : start + L_pre - 1 <= lo).
  {
    unfold ValidNodeFields, ValidNode in H50; cbn in H50.
    lia.
  }
  Exists chosen_2; Exists vals_out; Exists starts_out; Exists los_out; Exists his_out.
  Exists bests_out; Exists slots_out; Exists ans_2; Exists st_slots_2; Exists ps_2.
  ...
  - split_pures.
    all: dump_pre_spatial; try assumption; try lia.
    + unfold FrontierPushFields in H2.
      eapply frontier_split_push_single_pending_forms_frontier; eauto.
Qed. 
...
Proof.
  ...
  assert (Hright_int : -2147483648 <= right_value <= 2147483647).
  {
    unfold ValidNodeFields, ValidNode in H42; cbn in H42.
    destruct H42 as [_ [_ [_ [_ [_ [_ [_ [_ [Hvalue _]]]]]]]]].
    rewrite Hvalue.
    eapply PrefixSums_diff_int_bounds; eauto; lia.
  }
  assert (Hstart_lo : start + L_pre - 1 <= lo).
  {
    unfold ValidNodeFields, ValidNode in H43; cbn in H43.
    lia.
  }
  Exists chosen_2; Exists vals_out; Exists starts_out; Exists los_out; Exists his_out.
  Exists bests_out; Exists slots_out; Exists ans_2; Exists st_slots_2; Exists ps_2.
...
Proof.
  ...
  assert (Hleft_int : -2147483648 <= left_value <= 2147483647).
  {
    unfold ValidNodeFields, ValidNode in H41; cbn in H41.
    destruct H41 as [_ [_ [_ [_ [_ [_ [_ [_ [Hvalue _]]]]]]]]].
    rewrite Hvalue.
    eapply PrefixSums_diff_int_bounds; eauto; lia.
  }
  assert (Hstart_lo : start + L_pre - 1 <= lo).
  {
    unfold ValidNodeFields, ValidNode in H45; cbn in H45.
    lia.
  }
  Exists chosen_2; Exists vals_out; Exists starts_out; Exists los_out; Exists his_out.
  Exists bests_out; Exists slots_out; Exists ans_2; Exists st_slots_2; Exists ps_2.
  ...
  - split_pures.
    all: dump_pre_spatial; try assumption; try lia.
    + unfold FrontierPushFields in H3.
      eapply frontier_split_push_single_pending_forms_frontier; eauto.
Qed. 
...
Proof.
  pre_process.
  Exists chosen_2; Exists vals_2; Exists starts_2; Exists los_2.
  Exists his_2; Exists bests_2; Exists slots_2; Exists ans_2.
  Exists st_slots_2; Exists ps_2.
  split_pure_spatial.
  - repeat cancel.
  - pose proof H32 as Hvalid.
    unfold FrontierSplitState in H24; simpl in H24.
    unfold ValidNodeFields, ValidNode in Hvalid; cbn in Hvalid.
...
Proof.
  ...
  all: unfold PrefixSums, PrefixArrayPrefix in H8; lia.
Qed. 
...
Proof.
  ...
  split_pures.
  all: dump_pre_spatial.
  all: unfold PrefixSums, PrefixArrayPrefix in H8.
  all: unfold SparseArgmaxBuilt in H10.
