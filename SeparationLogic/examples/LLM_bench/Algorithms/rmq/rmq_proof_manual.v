...
Proof.
  ...
  - split_pures.
    all: dump_pre_spatial; try lia; try assumption;
      ...
      try (eapply STZeroPrefix_replace_zero_step; [eassumption | rewrite H7; lia]).
Qed.
...
Proof.
  ...
  assert (Hhalf_pos : 0 < half) by
    (rewrite H10; unfold Power2; apply Z.pow_pos_nonneg; lia).
  assert (Hlen_half : len = 2 * half) by
    (rewrite H11, H10; unfold Power2;
  ...
  {
    unfold STBuiltBeforeLevel in H14.
    destruct H14 as (_ & _ & _ & _ & _ & Hbuilt).
...
Proof.
  ...
  - split_pures.
    all: dump_pre_spatial; try solve [auto | lia].
    all: try solve
      [ unfold QueryLogFinalState; split; [exact H13|];
        unfold QueryLogLoopState in H13;
        destruct H13 as (_ & _ & _ & _ & Hk_nonneg & _ & Hpow & _);
        unfold Power2 in *;
        replace (k + 1) with (Z.succ k) by lia;
        rewrite Z.pow_succ_r by lia;
        rewrite <- Hpow; lia
      | unfold QueryLogLoopState in H13; intuition nia
      | unfold STBuilt, STBuiltBeforeLevel in H12;
        destruct H12 as (_ & _ & _ & _ & _ & Hbuilt);
        apply Hbuilt;
        unfold QueryLogLoopState in H13; intuition nia ].
Qed. 
