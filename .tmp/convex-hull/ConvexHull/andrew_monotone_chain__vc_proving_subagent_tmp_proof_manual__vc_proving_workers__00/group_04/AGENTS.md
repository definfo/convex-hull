...
- Do NOT inspect compile-only dependency paths with shell tools or editor reads. In particular, do not run `cat`, `sed`, `rg`, `grep`, `find`, `ls`, `head`, `tail`, or similar commands on:
  ...
  - `/home/definfo/Develop/qcp/convex-hull/.tmp/convex-hull/ConvexHull/andrew_monotone_chain__vc_proving_subagent_tmp_proof_manual__vc_proving_workers__00/group_04/case_deps/convex_hull`
...
## Assigned Goals
- `proof_of_partition_xy_points_return_wit_1` (`goal_19__proof_of_partition_xy_points_return_wit_1.v`)
- `proof_of_partition_xy_points_return_wit_2` (`goal_20__proof_of_partition_xy_points_return_wit_2.v`)

## Proof Group Notes
- proof_group_id: partition_finish
- grouping_source: vc-checking-group-plan
- representative_witness: proof_of_partition_xy_points_return_wit_1
- natural_language_proof_pattern: Convert scan invariant at loop exit into point_xy_partitioned_at, either by swapping pivot into i+1 or by direct no-swap finish.
- shared_helper_candidates: point_xy_partition_scan_inv_finish_swap, point_xy_partition_scan_inv_finish_noswap
- proving_hints: Use loop exit j=high and split left/right partition ranges; preserve permutation and same-outside facts.
