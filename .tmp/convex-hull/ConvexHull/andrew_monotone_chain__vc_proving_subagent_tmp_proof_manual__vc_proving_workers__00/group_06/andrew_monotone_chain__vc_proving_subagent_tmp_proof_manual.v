...
Proof.
  unfold andrew_monotone_chain_entail_wit_1; try left.
  intros.
  Exists (@nil Point) pts_out.
  entailer!.
  - eapply andrew_lower_scan_inv_init_worker.
    + rewrite PreH1. lia.
    + exact PreH2.
    + eapply point_list_not_all_same_of_points_not_all_same_permutation_worker;
        eauto.
...
Proof.
  unfold andrew_monotone_chain_entail_wit_2; try left.
  intros.
...
Proof.
  unfold andrew_monotone_chain_partial_solve_wit_8_pure; try left.
  intros.
  entailer!.
  all:
    unfold andrew_lower_scan_inv in PreH20;
    destruct PreH20 as [_ [Htop [_ [Hbound_lower _]]]];
    rewrite Htop in *;
    try (eapply Point_Order.point_bound; eapply points_in_bound_Znth; eauto; lia);
    try (eapply Point_Order.point_bound_sub; eapply points_in_bound_Znth; eauto; lia);
    eapply Point_Order.point_bound; eapply points_in_bound_Znth; eauto; lia.
Qed.
