...
COQC_TRANSIENT_RETRIES = 0
TRANSIENT_COQC_SIGNALS: set[int] = set()
...
def normalize_overlay_strategy_imports(path: Path) -> None:
    """Compatibility shim for copied r3 tooling.

    The upstream script version calls this hook while copying overlay
    dependencies, but the function is missing from this checkout.  This case
    imports the point-array strategy modules by their namespaced logical paths,
    so no bare strategy import rewrite is needed here.
    """
    return None
