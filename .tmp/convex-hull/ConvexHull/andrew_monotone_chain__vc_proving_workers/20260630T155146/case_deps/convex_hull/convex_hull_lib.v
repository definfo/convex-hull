...
Lemma point_xy_partitioned_at_preserved_by_left :
...
Lemma point_xy_partitioned_at_preserved_by_right :
  forall l l1 left right p,
    point_permutation l l1 ->
    0 <= left ->
    point_same_outside_range l l1 (p + 1) right ->
...
Proof.
  intros a b H.
  unfold point_leftdown, point_cmp_leftdown, x, y in *.
  destruct H as [H | [Hx Hy]];
  repeat match goal with
  | |- context [Z_lt_dec ?a ?b] => destruct (Z_lt_dec a b); simpl
  | |- context [Z_gt_dec ?a ?b] => destruct (Z_gt_dec a b); simpl
...
Proof.
  intros a b H.
  destruct (Z_lt_ge_dec (point_cmp_leftdown a b) 0) as [Hlt | Hge].
  - apply point_cmp_leftdown_lt_point_leftdown.
    exact Hlt.
  - assert (Hzero : point_cmp_leftdown a b = 0) by lia.
    unfold point_cmp_leftdown, point_leftdown, x, y in *.
    repeat match goal with
    | H : context [Z_lt_dec ?a ?b] |- _ => destruct (Z_lt_dec a b); simpl in H
    | H : context [Z_gt_dec ?a ?b] |- _ => destruct (Z_gt_dec a b); simpl in H
...
Proof.
  ...
  - destruct (Z.eq_dec j left) as [-> | Hjneq].
    + apply point_leftdown_point_cmp_leftdown_le.
      apply point_leftdown_refl.
    + assert (Hlt :
          point_cmp_leftdown (Znth left l default_point)
            (Znth j l default_point) < 0).
      {
        eapply (Forall_sublist_lookup_point
                  (fun x => point_cmp_leftdown
                    (Znth left l default_point) x < 0)
                  l (left + 1) (right + 1) j);
          try eassumption; lia.
      }
      lia.
    ...
    + destruct Hpart as [_ [Hleftpart Hrightpart]].
      ...
      * assert (Hip :
        ...
        -- assert (Hpj :
           ...
           eapply point_cmp_leftdown_le_lt_trans; eauto.
    + apply Hsorted_right; lia.
Qed.
