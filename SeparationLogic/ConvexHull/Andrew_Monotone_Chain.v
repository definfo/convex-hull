...
Proof.
  unfold leftdown, x, y, point_in_triangle, ccw,
         Record_Geo_Vec.left_than, Record_Geo_Vec.left_equal,
         colinear, Record_Geo_Vec.parallel, at_mid,
         Record_Geo_Vec.backward_or_perp, Record_Geo_Vec.cross_prod,
         Record_Geo_Vec.dot_prod, build_vec.
...
Lemma baseline_left_equal_not_ccw : forall first last q,
  Record_Geo_Vec.left_equal (build_vec first last) (build_vec first q) ->
  ~ ccw first last q.
Proof.
  unfold ccw, Record_Geo_Vec.left_than, Record_Geo_Vec.left_equal,
         Record_Geo_Vec.cross_prod, build_vec.
  simpl; intros; nia.
Qed.
...
Lemma opposite_left_equal_colinear : forall p q r,
  Record_Geo_Vec.left_equal (build_vec p r) (build_vec p q) ->
  Record_Geo_Vec.left_equal (build_vec r p) (build_vec r q) ->
  colinear q p r.
Proof.
  unfold Record_Geo_Vec.left_equal, Record_Geo_Vec.left_than,
         colinear, Record_Geo_Vec.parallel, Record_Geo_Vec.cross_prod,
         build_vec.
  simpl; intros; nia.
Qed.
...
Lemma leftdown_colinear_between_at_mid : forall p q r,
  ...
  at_mid q p r.
Proof.
  unfold colinear, Record_Geo_Vec.parallel, leftdown,
...
Fixpoint point_left_of_path_edges_from
  ...
  match path with
  | p :: rest =>
      Record_Geo_Vec.left_equal (build_vec prev p) (build_vec prev q) /\
...
Lemma left_equal_pop_shortcut :
  forall q p t s,
  ~ ccw s t p ->
  Record_Geo_Vec.left_equal (build_vec p t) (build_vec p q) ->
  Record_Geo_Vec.left_equal (build_vec t s) (build_vec t q) ->
  Record_Geo_Vec.left_equal (build_vec p s) (build_vec p q).
Proof.
  unfold ccw, Record_Geo_Vec.left_than, Record_Geo_Vec.left_equal,
         Record_Geo_Vec.cross_prod, build_vec.
  simpl; intros; nia.
...
Lemma left_equal_push_support_lower :
  ...
  ccw s t a ->
  Record_Geo_Vec.left_equal (build_vec t s) (build_vec t q) ->
  Record_Geo_Vec.left_equal (build_vec a t) (build_vec a q).
Proof.
  unfold leftdown, ccw, Record_Geo_Vec.left_than,
         Record_Geo_Vec.left_equal, Record_Geo_Vec.cross_prod, build_vec,
         x, y.
  simpl.
...
Lemma left_equal_pop_support_lower :
  ...
  ~ ccw s t a ->
  Record_Geo_Vec.left_equal (build_vec t s) (build_vec t q) ->
  Record_Geo_Vec.left_equal (build_vec a s) (build_vec a q).
Proof.
  unfold leftdown, ccw, Record_Geo_Vec.left_than,
         Record_Geo_Vec.left_equal, Record_Geo_Vec.cross_prod, build_vec,
         x, y.
...
Lemma left_equal_to_endpoint : forall p q,
  Record_Geo_Vec.left_equal (build_vec p q) (build_vec p p).
Proof.
  unfold Record_Geo_Vec.left_equal, Record_Geo_Vec.cross_prod, build_vec.
  simpl; intros; nia.
Qed.

Lemma left_equal_to_edge_endpoint : forall p q,
  Record_Geo_Vec.left_equal (build_vec p q) (build_vec p q).
Proof.
  unfold Record_Geo_Vec.left_equal.
  intros; rewrite Record_Geo_Vec.cross_prod_self; lia.
Qed.

Lemma ccw_rev_edge_left_equal : forall p q r,
  ccw p q r ->
  Record_Geo_Vec.left_equal (build_vec q p) (build_vec q r).
Proof.
  unfold ccw, Record_Geo_Vec.left_than, Record_Geo_Vec.left_equal,
         Record_Geo_Vec.cross_prod, build_vec.
  simpl; intros; nia.
...
Lemma point_in_hull_edges_two_path_segment :
  forall q first last,
  Record_Geo_Vec.left_equal (build_vec first last) (build_vec first q) ->
  Record_Geo_Vec.left_equal (build_vec last first) (build_vec last q) ->
  leftdown first q ->
  leftdown q last ->
